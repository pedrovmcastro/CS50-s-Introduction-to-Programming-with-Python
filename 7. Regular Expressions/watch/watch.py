...
def parse(s):
    """
    Extracts a YouTube URL from the value of the src attribute of an iframe element in the given HTML string.

    :param s: HTML string containing an iframe element with a YouTube URL as its src attribute.
    :return: shortened youtu.be equivalent of the extracted YouTube URL, or None if no valid URL is found.
    """
    if matches := re.search(r'"(https?://(?:www\.)?youtube\.com/embed/[0-9A-Za-z_-]{11})"', s):
        return matches.group(1).replace("www.", "").replace("http:", "https:").replace("youtube.com/embed", "youtu.be")
